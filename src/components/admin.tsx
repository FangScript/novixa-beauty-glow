    ...
    <div className="min-h-screen bg-[#f6f1eb] text-[#211b18]">
      ...
      >
        <div className="flex items-center justify-between">
          <Link to="/admin" className="font-display text-3xl">
            NOVIXA
          </Link>
          <button
            className="lg:hidden"
        ...
        <nav className="mt-8 space-y-1">
          {nav.map(([label, href, Icon]) => (
            <Link
              ...
              to={href}
              onClick={() => setMenuOpen(false)}
              activeProps={{ className: "bg-[#51413a] text-[#fff8f1]" }}
              className="flex items-center gap-3 px-3 py-2.5 text-xs text-[#cfc3bb] transition-colors hover:bg-[#3d302b] hover:text-white"
            >
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-[#d9cec5] bg-[#f6f1eb]/95 px-5 backdrop-blur md:px-8">
          <button
...
export function MetricCard({
  ...
  return (
    <div
      className={`border p-5 ${tone === "dark" ? "border-[#211b18] bg-[#211b18] text-white" : "border-[#d9cec5] bg-white/50"}`}
    >
      <p
        className={`text-[9px] uppercase tracking-[0.18em] ${tone === "dark" ? "text-[#c9a982]" : "text-[#8f5d48]"}`}
      >
        {label}
      </p>
  ...
  return (
    <div className="flex flex-col gap-3 border-y border-[#d9cec5] py-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex gap-2">
        ...
        <Button className="h-10 rounded-none text-[10px] uppercase tracking-[0.12em]">
          <Plus size={14} /> Create
        </Button>
      </div>
    </div>
...
export function ProductRow({ product }: { product: Product }) {
  return (
    <tr className="border-b border-[#e7ddd5] last:border-0">
      <TableCell>
        <div className="flex items-center gap-3">
      ...
      <TableCell>
        <span
          className={`inline-flex px-2 py-1 text-[9px] uppercase ${product.stock <= 5 ? "bg-[#f1d2c9] text-[#8f3f2d]" : "bg-[#dfe8d9] text-[#4b6742]"}`}
        >
      ...
      <TableCell className="text-right">
        <Link
          ...
          className="text-[#8f5d48] underline"
        >
          View
        </Link>
      </TableCell>
    ...
    >
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-[#d9cec5] bg-[#f6f1eb] p-6 shadow-2xl md:p-8">
        <div className="flex items-start justify-between gap-5 border-b border-[#d9cec5] pb-5">
          ...
          <button
            onClick={onClose}
            className="text-2xl leading-none text-[#776a61]"
